...
class A2UIPlaygroundMenuViewController: UIViewController {
    ...
    private func loadTemplatesFromZip() {
        // Get Templates.zip path from Bundle
        guard let zipPath = Bundle.main.path(forResource: "Templates", ofType: "zip") else {
            print("⚠️ Templates.zip file not found")
            return
        }
        
        // Create temporary directory path
        tempDir = NSTemporaryDirectory() + "Templates/"
        
        // If temporary directory exists, delete it first
        ...
        guard SSZipArchive.unzipFile(atPath: zipPath, toDestination: tempDir) else {
            print("⚠️ Failed to unzip Templates.zip")
            return
        }
        
        // Traverse directory to get all component type folders
        guard let contents = try? fileManager.contentsOfDirectory(atPath: tempDir) else {
            print("⚠️ Unable to read temporary directory contents")
            return
        }
        
        // Filter valid component folders (exclude system files)
        componentTypes = contents.filter { item in
            ...
            let fullPath = tempDir + item
            fileManager.fileExists(atPath: fullPath, isDirectory: &isDirectory)
            return isDirectory.boolValue
        }.sorted()
        
        print("✅ Loaded \(componentTypes.count) component types")
    ...
    private func loadVariants(for componentType: String) {
        let fileManager = FileManager.default
        let componentPath = tempDir + componentType
    ...
    private func loadTemplateFiles(componentType: String, variant: String) {
        let fileManager = FileManager.default
        let variantPath = tempDir + componentType + "/" + variant
